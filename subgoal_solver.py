...
def objective(opt_vars,
    ...
    """
    Computes the total cost for a given set of optimization variables.
    This function evaluates various costs such as collision cost, consistency with the initial pose, inverse kinematics feasibility, grasp metrics, and constraint violations. It aggregates these costs to compute a total cost that can be used for optimization purposes.
        opt_vars (numpy.ndarray): Optimization variables representing the pose transformation parameters.
        og_bounds (tuple): Environment boundaries (min_bound, max_bound) in the omnigibson environment.
        keypoints_centered (numpy.ndarray): Centered keypoints in the object's local frame.
        keypoint_movable_mask (numpy.ndarray): Boolean mask indicating movable keypoints.
        goal_constraints (list of callables): List of goal constraint functions generated by VLM.
        path_constraints (list of callables): List of path constraint functions generated by VLM.
        sdf_func (callable): Signed distance function used to compute collision costs.
        collision_points_centered (numpy.ndarray): Centered collision points in the object's local frame.
        init_pose_homo (numpy.ndarray): Initial homogeneous pose matrix of the object.
        ik_solver (IKSolver): Inverse kinematics solver instance.
        initial_joint_pos (numpy.ndarray): Initial joint positions of the robot manipulator.
        reset_joint_pos (torch.Tensor): Joint positions representing the robot's reset configuration.
        is_grasp_stage (bool): Flag indicating whether the current stage is a grasping stage.
        return_debug_dict (bool, optional): If True, returns a dictionary containing intermediate cost components for debugging.
    Returns:
        float or tuple: If `return_debug_dict` is False, returns the total cost as a float. If True, returns a tuple `(cost, debug_dict)` where `debug_dict` contains intermediate cost components.
    Notes:
        - The collision cost penalizes poses where the object is in collision, computed using the signed distance function.
        - The initial pose cost penalizes deviation from the initial pose, encouraging consistency.
        - The inverse kinematics (IK) cost approximates reachability by considering the number of descent iterations and regularization terms.
        - The reset regularization cost penalizes deviation from a specified reset joint position.
        - If `is_grasp_stage` is True, an additional grasp cost is computed to encourage a preferred grasping orientation.
        - Goal and path constraint costs penalize violations of constraints provided by VLM, such as maintaining certain keypoint relationships.
    """
    debug_dict = {}
    ...
    if collision_points_centered is not None:
        collision_cost = 0.8 * \
            calculate_collision_cost(
                opt_pose_homo[None], sdf_func, collision_points_centered, 0.10)
        debug_dict['collision_cost'] = collision_cost
        cost += collision_cost

    # stay close to initial pose
    init_pose_cost = 1.0 * \
        consistency(opt_pose_homo[None], init_pose_homo[None], rot_weight=1.5)
    ...
    # ik_cost = 0 * (ik_result.num_descents / max_iterations)
    debug_dict['ik_feasible'] = ik_result.success
    debug_dict['ik_pos_error'] = ik_result.position_error
    debug_dict['ik_cost'] = ik_cost
    cost += ik_cost
    if ik_result.success:
        reset_reg = np.linalg.norm(
            ik_result.cspace_position[:-1] - reset_joint_pos[:-1].detach().numpy())
    ...
    # formulate the grasp cost to customize the desired grasp pose
    if is_grasp_stage:
        preferred_dir = np.array([0, 0, -1]) # calculate the cos metric between z-axis rotated with the original neg z-axis
        # grasp_cost = -np.dot(opt_pose_homo[:3, 0], preferred_dir) + 1  # [0, 1]
        grasp_cost = -np.dot(opt_pose_homo[:3, 2], preferred_dir) + 1  # [0, 1] maximize the cos metric
        grasp_cost += -np.dot(opt_pose_homo[:3, 1], np.array([1, 0, 0])) + 1
    ...
    if goal_constraints is not None and len(goal_constraints) > 0:
        ...
        transformed_keypoints = transform_keypoints(
            opt_pose_homo, keypoints_centered, keypoint_movable_mask)
        subgoal_violation = []
        for constraint in goal_constraints:
            violation = constraint(
                transformed_keypoints[0], transformed_keypoints[1:])
    ...
    if path_constraints is not None and len(path_constraints) > 0:
        ...
        # path_constraint_cost = 0*path_constraint_cost
        path_constraint_cost = 200*path_constraint_cost
...
class SubgoalSolver:
    """
    optimization variables: the next step ee_pose for subgoal
    """
    ...
    def _warmup(self):
        ...
        self.solve(ee_pose, keypoints, keypoint_movable_mask, goal_constraints,
                   path_constraints, sdf_voxels, collision_points, True, None, from_scratch=True)
        self.last_opt_result = None
    ...
    def _check_opt_result(self, opt_result, debug_dict):
        ...
        if debug_dict['subgoal_violation'] is not None:
            goal_constraints_results = np.array(
                debug_dict['subgoal_violation'])
            opt_result.message += f'; goal_constraints_results: {goal_constraints_results} (higher is worse)'
            goal_constraints_satisfied = all(
                [violation <= self.config['constraint_tolerance'] for violation in goal_constraints_results])
        ...
        if debug_dict['path_violation'] is not None:
            ...
            path_constraints_satisfied = all(
                [violation <= self.config['constraint_tolerance'] for violation in path_constraints_results])
    ...
    def _center_collision_points_and_keypoints(self, ee_pose_homo, collision_points, keypoints, keypoint_movable_mask):
        """
        using ee_pose_homo to center collision points and keypoints.
        centering_transform: from world frame to ee frame (inv of ee_pose, ee_pose convert points from ee frame to world frame)

        """
        centering_transform = np.linalg.inv(ee_pose_homo)
        collision_points_centered = np.dot(
            collision_points, centering_transform[:3, :3].T) + centering_transform[:3, 3]
        keypoints_centered = transform_keypoints(
            centering_transform, keypoints, keypoint_movable_mask)
    ...
    def solve(self,
        ...
        if collision_points is not None and collision_points.shape[0] > self.config['max_collision_points']:
            collision_points = farthest_point_sampling(
        ...
        ee_pose_euler = np.concatenate(
            [ee_pose[:3], T.quat2euler(ee_pose[3:])])
        ...
        og_bounds = [(b_min, b_max) for b_min, b_max in zip(np.concatenate(
            [pos_bounds_min, rot_bounds_min]), np.concatenate([pos_bounds_max, rot_bounds_max]))]
        ...
        _, debug_dict = objective(
            opt_result.x, *aux_args, return_debug_dict=True)
